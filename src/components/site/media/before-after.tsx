...
export function BeforeAfter({ before, after, alt }: { before: string; after: string; alt: string }) {
  ...
  return (
    <div className="relative aspect-[16/9] select-none overflow-hidden rounded-(--radius-xl)">
      ...
      <span className="t-meta absolute top-4 left-4 rounded-full bg-black/50 px-2.5 py-1 text-white backdrop-blur">BEFORE</span>
      <span className="t-meta absolute top-4 right-4 rounded-full bg-orange px-2.5 py-1 text-white">AFTER</span>
