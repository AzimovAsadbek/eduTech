...
export function CourseApply({ course, branches }: Props) {
  ...
  return (
    <div id="ariza" className="glass relative overflow-hidden rounded-(--radius-xl) p-6 sm:p-8">
      ...
      <p className="t-eyebrow text-orange">Kursga yozilish</p>
      <h2 className="t-h3 mt-2">{course.title}</h2>
      <p className="mt-1 text-sm text-(--fg-muted)">
        {course.durationLabel}
        {course.priceLabel ? ` · ${course.priceLabel}` : " · narx konsultatsiyada"}
      </p>
      <div className="mt-6">
        <LeadForm type="EDUCATION" courses={[{ value: course.slug, label: course.title }]} branches={branches} defaultCourseSlug={course.slug} source={`course:${course.slug}`} submitLabel="Ariza yuborish" />
      </div>
    </div>
  );
}
