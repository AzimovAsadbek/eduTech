...
import { RESOURCES, type Option, type RelationKey, type ResourceKey } from "./config";
...
export function ResourceForm({ resource, mode, item, relations, role }: ResourceFormProps) {
  ...
  const applyIssues = useCallback(
    (issues: { path: string; message: string }[]) => {
      ...
      setErrors(map);
      firstErrorRef.current = issues[0]?.path.split(".")[0] ?? null;
      const el = document.querySelector<HTMLElement>(`[data-field="${firstErrorRef.current}"]`);
  ...
  const save = async (statusOverride?: ContentStatus) => {
    ...
    if (!parsed.success) {
      applyIssues(parsed.error.issues.map((i) => ({ path: i.path.join("."), message: localizeIssue(i) })));
    ...
    try {
      ...
      if (isApiError(e) && e.details.length) applyIssues(e.details.map((d) => ({ path: d.path, message: localizeIssue({ code: "custom", message: d.message }) })));
  ...
  return (
    ...
    >
          ...
          <Card key={section.title}>
            ...
            <CardBody className="grid gap-4 sm:grid-cols-2">
              {section.fields.map((f) => (
                <div key={f.name} data-field={f.name} className={f.span === 2 || ["textarea", "image", "string-list", "json-list", "socials", "multiselect"].includes(f.type) && f.span !== 1 ? "sm:col-span-2" : undefined}>
                  <FieldRenderer
                    field={f}
                    value={values[f.name]}
      ...
      <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
        <Card>
          <CardHeader title={mode === "create" ? "Yaratish" : "Nashr"} actions={ui.hasStatus && mode === "edit" && typeof item?.status === "string" ? <ContentStatusBadge status={item.status as ContentStatus} /> : null} />
          <CardBody className="space-y-4">
            {ui.hasStatus ? (
              <div data-field="status">
                <p className="mb-1.5 text-[13px] font-semibold text-ink">Holat</p>
                <Segmented<ContentStatus> label="Holat" value={status} options={STATUS_OPTIONS} onChange={(v) => setValue("status", v)} className="w-full [&>button]:flex-1" />
                {errors.status ? <p className="mt-1 text-[13px] text-danger">{errors.status}</p> : null}
              </div>
            ) : null}
            {ui.panelToggle ? <Switch label={ui.panelToggle.label} hint={ui.panelToggle.hint} checked={Boolean(values[ui.panelToggle.name])} onChange={(v) => setValue(ui.panelToggle!.name, v)} /> : null}
            <div data-field="order">
              <Input type="number" inputMode="numeric" min={0} label="Tartib" hint="Kichik raqam oldinroq chiqadi" value={String(values.order ?? "0")} onChange={(e) => setValue("order", e.target.value)} error={errors.order} />
            </div>
            <div className="flex flex-col gap-2 border-t border-(--line) pt-4">
              ...
              {dirty ? <p className="t-meta text-center text-orange">Saqlanmagan oʻzgarishlar bor</p> : null}
            </div>
          </CardBody>
        </Card>
        ...
        {id ? (
          <Card>
            ...
            <CardBody className="space-y-2">
              ...
              <dl className="t-meta space-y-1 border-t border-(--line) pt-3 text-muted">
                <div className="flex justify-between gap-2">
                  <dt>ID</dt>
                  <dd className="truncate text-ink">{id}</dd>
                </div>
